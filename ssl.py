        ...
        for x in res.replace('\r','').replace('\n','##').replace('*','#$*').split('#$'):
            if x.startswith('* Certificates Information:'):
                ...
                if re.search('Certificate is NOT Trusted',x,re.IGNORECASE) or re.search('certificate untrusted',x,re.IGNORECASE):
                    ...
                    imp='The X.509 certificate cannot be trusted. This situation can occur in three different ways, in which the chain of trust can be broken, as stated below :\n- First, the top of the certificate chain sent by the server might not be descended from a known public certificate authority. This can occur either when the top of the chain is an unrecognized, self-signed certificate, or when intermediate certificates are missing that would connect the top of the certificate chain to a known public certificate authority.\n- Second, the certificate chain may contain a certificate that is not valid at the time of the scan. This can occur either when the scan occurs before one of the certificate notBefore dates, or after one of the certificates notAfter dates.\n- Third, the certificate chain may contain a signature that either did not match the certificate information or could not be verified. Bad signatures can be fixed by getting the certificate with the bad signature to be re-signed by its issuer.\nIf the remote host is a public host in production, any break in the chain makes it more difficult for users to verify the authenticity and identity of the web server. This could make it easier to carry out man-in-the-middle attacks against the remote host.'
                    sol='Purchase or generate a proper SSL certificate for this service.'
                    ref='CVE-2020-35733'
